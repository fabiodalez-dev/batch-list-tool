...
final class TemplateGenerator
{
    public const array AUTHORITY_HEADERS = [
        ...
        // Descriptive fields. "Notes" existed on the form and in the database
        // all along but had no column here, so it could be typed and never
        // imported.
        'Authorised form of name', 'Functions, occupations and activities',
        'Level of detail', 'Status', 'Rules and/or conventions',
        'Date of creation', 'Creator of record', 'Notes',
    ];
    ...
    /**
     * Generator version — stamped into the hidden metadata sheet.
     *
     * Provenance, not a gate: nothing reads it back at upload time, so a
     * sheet generated by 1.13.0 imports exactly like one generated today.
     * Its use is forensic — when a sheet does not import as expected, the
     ...
     */
    /**
     * How far down the dropdowns reach. Excel needs an explicit range; a
     * thousand rows covers any sheet the NAf has sent so far without
     * bloating the file.
     */
    private const int VALIDATED_ROWS = 1000;

    /**
     * Columns that accept only a closed list of values.
     *
     * Keyed by entity, then by the header exactly as it appears in row 1.
     * Those columns get a dropdown, so an unlisted spelling is refused in
     * the sheet instead of failing the row at upload time — "Full" is not
     * "Full Level", and the operator has no other way to know that.
     *
     ...
     * everything below blank) plus a hidden metadata sheet recording how and
     ...
     * fields were present when the template was generated.
     *
     * Nothing validates this sheet at upload time — no caller reads it back.
     * It exists to be inspected by hand when a sheet misbehaves, which is how
     * a stale download gets told apart from a current one.
     *
    ...
    private static function buildSpreadsheet(string $entity, array $headers): Spreadsheet
    {
        ...
        foreach (self::ENUM_COLUMNS[$entity] ?? [] as $header => $options) {
            ...
            // Blank is legitimate: every one of these fields is optional.
            $validation->setAllowBlank(true);
            $validation->setShowDropDown(true);
            $validation->setShowErrorMessage(true);
            $validation->setErrorTitle('Value not allowed');
            $validation->setError(sprintf(
                '%s must be one of: %s.',
                $header,
                implode(', ', $options)
            ));
            // An inline Excel list is one quoted, comma-separated string.
            $validation->setFormula1('"' . implode(',', $options) . '"');

            $sheet->setDataValidation(
                $letter . '2:' . $letter . self::VALIDATED_ROWS,
                $validation
            );
        }
        ...
        // Sheet 1: hidden provenance. The operator never sees it and no code
        // reads it back; it is here to be read by hand when a sheet arrives
        // that does not import as expected.
